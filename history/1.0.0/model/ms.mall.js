...
define(function(require, exports, module) {
	...
	return {
		...
		specification : {
			...
			* @param {{type:function,have:false}}  回调方法 返回值（returnJson）此时返回的是选中一组规格后，当前规格组的属性
			* @examples 
			* ...
			...
			*/
			 ...
			 * @param {{type:int,have:false}} detailId 商品规格明细id
			 * @param {{type:function,have:true}}  回调方法 返回值（returnJson）
			 * @examples 
			 * ...
		...
		product : {
			 ...
			 * @param {{type:string,have:false}} spec 规格筛选： 多个规格筛选用","分割,相同规格多选用@分割。如 spec=尺寸:1寸@2寸,形状:圆形
			 * @param {{type:string,have:false}} price 按价格排序，格式：min-max, 没有上线或下限可以缺省，没有"-" 表示精确价格筛选。如 &price=0.3-234
			 * @param {{type:string,have:false}} dataType 返回的数据类型,默认为html格式，根据模板生成的html页面，json 异步返回json数据，在页面需要获取到Json数据进行操作时可选择为json。如 dataType=json
			 * @param {{type:string,have:false}} sort 声明 升序降序  排序字段-方式,不写方式默认降序 desc。如 sort=sale-asc 按销量排序
			 * @param {{type:function,have:false}}  回调方法 返回值（returnJson）
			 * @examples 
			 * ...
			 * mmall.product.product.search($("form").serialize(),function(returnJson){
			 * 	alert(JSON.stringify(returnJson.resultMsg));	
			 * });
			 * ...
			 * @function
			 * 待补充
			 * @return {{type:function,have:true}}  待补充
			 */
		...
		people : {
			...
			mall : {
				...
				cart : {
					...
					* @param {{type:string,have:false}}  cartIds 购物车编号，多个用","隔开
					* @param {{type:function,have:true}}  回调方法 返回值（returnJson）
					* @examples 
					* ...
