define(function(require, exports, module) {
	...
	return {
		...
		* @return {{type:result}} true成功、false失败
	    */
	    cancelBind : function(data,func){
			ajaxCfg.url = ms.base + "/cancelBind.do";
			if (ms.isEmpty(data)){
				return;
			};
			//将data参数转json
			...
			basic:{
				people:{
					...
					save:function(data,func){
						...
						ajaxCfg.url = ms.base + "/people/basic/people/save.do";
						ajaxCfg.params = data;
						ms.ajax(ajaxCfg, func);
					},
					/**
					...
					del:function(data,func){
						...
						ajaxCfg.url = ms.base + "/people/basic/people/delete.do";
						ajaxCfg.params = data ;
						ms.ajax(ajaxCfg, func);	
					},
				}
		    ...
		    address : {
				...
				*/
				save : function(data,func){
					...
					if (validator.isNull(newnDataJson.peopleAddressProvince) && validator.isNull(newnDataJson.peopleAddressProvinceId)){
						ms.alert("请把所在区域填写完整");
						return;
					};
