...
$(function() {
    ...
    var ck = ms.queryString("ck");
    if(ck=="" || ck==null || ck=="null") {
    	ck = "";
    }
    if (msurl) {
    	$(".wellcome").hide();
    	...
    	if (!$('.easyui-tabs').tabs('exists', title)) {
	        $('.easyui-tabs').tabs('add', {
	            title: title,
	            content: '<iframe src="'+decodeURIComponent(msurl)+'" frameborder="0" height="100%" width="100%" id="mainFrame" name="mainFrame"></iframe>',
	            closable: true,
	        });
    	}
	...
	$.ajax({
	    type:"post",
	    dataType: "json",
	    url: msotreClientPath+"/upgrader/sync.do?ck="+ck,
	    beforeSend:function(){
